	...
	namespace leetcode
	{
		...
		***********************************************************************/
		...
		struct NodeMeta {
			int pathValue;
			TreeNode* parent;
			NodeMeta(int x, TreeNode* p) : pathValue(x), parent(p) {}
		};
		vector<vector<int>> pathSum(TreeNode* root, int sum) {
			...
			unordered_map<TreeNode*, NodeMeta*> data;
			data.insert({ root, new NodeMeta(root->val, NULL) });
			...
			while (!list.empty()) {
				...
				list.pop();
				if (c != NULL) {
					if (c->right == NULL && c->left == NULL && data[c]->pathValue == sum) {
						TreeNode* ptr = c;
						vector<int>* temp = new vector<int>();
						while (ptr != NULL) {
							temp->insert(temp->begin(), ptr->val);
							ptr = data[ptr]->parent;
						}
						a.push_back(*temp);
					}
					...
					list.push(c->right);
					if (c->left != NULL) {
						NodeMeta* d = new NodeMeta(data[c]->pathValue + c->left->val, c);
						data.insert({ c->left, d });
					}
					if (c->right != NULL) {
						NodeMeta* d = new NodeMeta(data[c]->pathValue + c->right->val, c);
						data.insert({ c->right, d });
					}
				}
			}
			return a;
		...
		//ran off test cases, saved solution here anyway
		//https://leetcode.com/problems/path-sum-ii/
		void PathSumII() {}

		/**********************************************************************
			Problem 15: Medium - 3sum
		...
		***********************************************************************/
		...
		/*long long int getUniqueKeyNum(long long int a, long long int b, long long int c, long long int len) {
			if (a < b) {
				if (a < c) {
					return b < c ? a + b * len + c * len * len : a + c * len + b * len * len;
				}
				return c + a * len + b * len * len;
			}
			else {
				if (b < c) {
					return a < c ? b + a * len + c * len * len : b + c * len + a * len * len;
				}
				return c + (b * len) + (a * len * len);
			}
		}*/

		void QuickSortManual(vector<int>& list) {

		}

		//DODGY SORT FROM THE PUBLIC!
		void publicSortArray(vector<int>& nums) {
			int n = nums.size();
			...
			//decoded this via debugging - create a list of size of max number difference
			vector<int> C(maxEl - minEl + 1);

			//then count each number of times an element repeats with distance respect to the minimum element
			for (int i : nums) ++C[i - minEl];
			int j = 0;
			for (int i = 0; i < C.size(); ++i) {
				int el = i + minEl;
				int& cnt = C[i];
				//use the mapper C to find the instances of this element and add them back to the array
				while (cnt--) nums[j++] = el;
			}
		}
		...
		vector<int> getPreparedList(vector<int>& nums, int len) {
			vector<int> zeroes;
			vector<int> left;
			left.reserve(len);
			vector<int> right;
			...
			publicSortArray(left);
			publicSortArray(right);
			...
			for (int i = 0; i < left.size(); ++i) {
				if (i > 1) {
					if (left[i] == left[i - 1] && left[i] == left[i - 2]) {
						continue;
					}
				}
			...
			for (int i = 0; i < right.size(); ++i) {
				if (i > 1) {
					if (right[i] == right[i - 1] && right[i] == right[i - 2]) {
						continue;
					}
				}
		...
		vector<vector<int>> threeSumRedo(vector<int>& nums) {
			int len = nums.size();
			if (len < 3) {
				return {};
			}
			...
			vector<int> list = getPreparedList(nums, nums.size());
			// vector<int> & list = nums;
			// publicSortArray(list);
			len = list.size();
			if (len < 3 || list.back() < 0 || list[0] > 0) {
				return {};
			}
			...
			for (int i = 0; i < len - 2; ++i) {
				...
				//same as last index creates the same target so skip
				...
				while (j < k) {
					if (list[j] + list[k] == target) {
						answers.push_back({ list[i], list[j], list[k] });
		...
		vector<int> GetSorted3SumList(vector<int>& list) {
			int len = list.size();
			auto left = vector<int>();
			auto right = vector<int>();
			auto center = vector<int>();
			int divider = 0;
			for (int i = 0; i < len; ++i) {
				int c = list[i];
				if (c < divider) {
			...
			answer.reserve(len);
			//merge the list -- cull any numbers that repeat more than 2 times and cull zeroes more than 3
			QuickSortManual(left);
			QuickSortManual(right);
			int leftSize = left.size();
			for (int i = 0; i < leftSize; ++i) {
				answer.push_back(left[i]);
			}
			int centerSize = center.size();
			for (int i = 0; i < centerSize && i < 3; ++i) {
				answer.push_back(center[i]);
			}
		...
		unsigned long long int getUniqueKeyNum(long a, long b, long c, long len) {
			if (a < b) {
				if (a < c) {
					return b < c ? a + b * len + c * len * len : a + c * len + b * len * len;
				}
		...
		vector<vector<int>> threeSum(vector<int>& nums) {
			...
			vector<int> sortedList = GetSorted3SumList(nums);
			crispyskies::utils::PrintVector(sortedList);
			vector<vector<int>> answers = {};
			return answers;
		}

		vector<vector<int>> threeSumCrapSolution(vector<int>& nums) {
			int len = nums.size();
			if (len < 3) {
				return {};
			}
			vector<vector<int>> answers = {};
			map<int, int> targetrefs;
			bool boolFoundZero = false;
			for (int i = 0; i < len; ++i) {
				int target = nums[i];
				if (boolFoundZero && target == 0) {
					continue;
				}
				if (target == 0) {
					boolFoundZero = true;
				}
				if (targetrefs.find(target) == targetrefs.end()) {
					targetrefs.insert({ target, i });
				}
			}
			unordered_map<unsigned long long int, bool> uniques;
			auto pairDupeCheck = unique_ptr<bool[]>{ new bool[len * len]{0} };
			for (int i = 0; i < len; ++i) {
				for (int j = 0; j < len; ++j) {
					if (i != j) {
						int pairKey = i <= j ? i + j * len : j + i * len;
						if (!pairDupeCheck[pairKey]) {
							pairDupeCheck[pairKey] = true;
							int target = -nums[i] - nums[j];
							if (targetrefs.find(target) != targetrefs.end()) {
								int t = targetrefs.at(target);
								if (t != i && t != j) {
									auto ukey = getUniqueKeyNum(nums[i], nums[j], nums[t], len);
									if (!uniques[ukey]) {
										uniques[ukey] = true;
										answers.push_back({ nums[i], nums[j], nums[t] });
									}
								}
							}
						}
		...
		//	//create a map of all possible answers
		//	map<int, int> targetrefs;
		//	for (int i = 0; i < len; ++i) {
		//		int target = nums[i];
		//		if (targetrefs.find(target) == targetrefs.end()) {
		//			targetrefs.insert({ target, i });
		//		}
		//		//targetrefs[target].push_back(i);
		//	}
		//	//LOG(targetrefs.size());

		//	//const int PRIME_NUMBER_MAGIC001 = 199783;
		//	LOG("threeSum:: starting first loop");
		//	vector<vector<int>> answers = {};
		//	unordered_map<long, bool> uniques;
		//	LOG("threeSum:: before loop");
		//	auto pairDupeCheck = unique_ptr<bool[]>{ new bool[len * len]{0} };
		//	for (int i = 0; i < len; ++i) {
		//		for (int j = 0; j < len; ++j) {
		//			if (i != j) {
		...
		//					if (targetrefs.find(target) != targetrefs.end()) {
		//						int t = targetrefs.at(target);
		//						long ukey = getUniqueKeyNum(nums[i], nums[j], nums[t], len);
		//						if (!uniques[ukey]) {
		//							uniques[ukey] = true;
		//							answers.push_back({ nums[i], nums[j], nums[t] });
		//						}
		...
		void ThreeSumProblem() {
			...
			//		LOG("testing function:: starting");
			//		auto funcName = func->first;
			//		auto matchFunc = func->second;
			//		auto yourAnswer = matchFunc(currentTest);
			//		LOG("testing function:: finished");
			//		//crispyskies::utils::PrintVector(yourAnswer);
			//	}
			//}
			...
			map<int, bool> pairDupeCheck;
			string test = pairDupeCheck[0] ? "yes" : "no";
			map<int, vector<int>> targetrefs;
			string test2 = targetrefs.find(0) != targetrefs.end() ? "yes" : "no";
			vector<int> test3 = targetrefs[150];
			test3.push_back(2);
			LOG(test3.size());
			test3.clear();
			LOG(test3.size());
			LOG(getUniqueKeyNum(1, 3000, 3000, 3000));*/
		}
